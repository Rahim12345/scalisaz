    ...
    <div class="page">
        ...
        <div class="content">
            <div class="mb-3 col-md-8 offset-md-2 table-responsive" style="overflow-x: hidden;padding: 0 10px !important;">
                <a href="{{ route('product.create') }}" class="btn btn-primary w-100">{{ __('static.elave_et') }}</a>
    ...
    <script>
        ...
        function getUsers(table,filter) {
            table.DataTable({
                ...
                columns: [
                    {data: 'esas_menu', name: 'esas_menu'},
                    {data: 'sub_menu_1', name: 'sub_menu_1'},
                    {data: 'sub_menu_2', name: 'sub_menu_2'},
                    {data: 'agt', name: 'agt'},
                    {data: 'action', name: 'action'}
                ],
                    createdRow: function( row, data, dataIndex ) {
                    $( row ).find('td:eq(0)').attr('data-label', "Əsas səhifə");
                    $( row ).find('td:eq(1)').attr('data-label', "Sub Menu 1");
                    $( row ).find('td:eq(2)').attr('data-label', "Sub Menu 2");
                    $( row ).find('td:eq(3)').attr('data-label', 'Məhsullar');
...
<script>
    ...
    <script>
        $(document).on('click', '.productDeleter', function () {
            var id = $(this).data('id');
            var table = $('#products');
            $.ajax({
                url: '/admin/mehsullar/product/'+id,
                type: 'DELETE',
                data: {
                    '_token': '{{ csrf_token() }}'
                },
